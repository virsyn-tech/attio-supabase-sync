#!/usr/bin/env python3
"""
Attio to Supabase Sync Script - Agent Reports Project
...
Environment Variables:
    ...
    AGENT_REPORTS_SUPABASE_URL - Supabase project URL
    AGENT_REPORTS_SUPABASE_KEY - Supabase service role key
...
SUPABASE_URL = os.getenv('AGENT_REPORTS_SUPABASE_URL')
SUPABASE_SERVICE_ROLE_KEY = os.getenv('AGENT_REPORTS_SUPABASE_KEY')
...
def validate_env():
    ...
    if not SUPABASE_URL:
        missing.append('AGENT_REPORTS_SUPABASE_URL')
    if not SUPABASE_SERVICE_ROLE_KEY:
        missing.append('AGENT_REPORTS_SUPABASE_KEY')
...
def transform_merchant(merchant: dict) -> dict:
    """Transform Attio merchant to Supabase rcti_merchant format (snake_case)."""
    ...
    return {
        'mid': mid,
        'trading_name': get_attribute_value(values, 'trading_name'),
        'qantas_plan': get_attribute_value(values, 'qantas_plan'),
        'eftpos_structure': get_attribute_value(values, 'debit_eftpos_structure'),
        'number_of_terminals': get_attribute_value(values, 'number_of_terminals'),
        'merchant_terminal_rental': get_attribute_value(values, 'merchant_terminal_rental'),
        'aps_terminal_rental': get_attribute_value(values, 'aps_terminal_rental'),
        'merchant_status': get_attribute_value(values, 'merchant_status'),
        'agent_name': agent.get('agent_name'),
        'agent_rate': agent.get('agent_commission_rate'),  # Already numeric in Attio
        'master_agent_name': master_agent.get('agent_name'),
        'master_agent_rate': master_agent_rate,  # Converted from text to numeric
        'status': get_attribute_value(values, 'account_status'),
        'owner_manager_name': person.get('name'),
...
def main():
    """Main sync function."""
    logger.info("=" * 60)
    logger.info("Starting Attio to Agent Reports Supabase sync")
    logger.info("=" * 60)
    ...
    # Upsert to Supabase
    logger.info("Upserting to Supabase (Agent Reports)...")
